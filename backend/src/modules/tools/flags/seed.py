"""Demo feature flags and an evaluation API key for the seeded admin.
...
from ....platform_sdk import KeyPermissionAction, KeyPermissionResource, issue_api_key, user_id_by_username
from . import service
...
async def _admin_user(db: AsyncSession) -> dict[str, Any]:
    user_id = await user_id_by_username(db, ADMIN_USERNAME)
    if user_id is None:
...
async def _seed_flags(db: AsyncSession, admin: dict[str, Any]) -> int:
    created = 0
    permissions = {PERM_FLAGS_WRITE}
...
async def _seed_api_key(db: AsyncSession, admin: dict[str, Any]) -> str | None:
...
async def seed_demo_data(db: AsyncSession) -> None:
    """Create the demo flags and the evaluation key, printing the key once."""
