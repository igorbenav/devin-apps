...
- Verify the KYC migration revision and tables before assuming startup/seed ran. If needed, from backend run `POSTGRES_SERVER=127.0.0.1 uv run alembic upgrade head` and `POSTGRES_SERVER=127.0.0.1 uv run python -m scripts.seed_tools`.
